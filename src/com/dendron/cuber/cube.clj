...
;(binding [*noisy?* true] (compile 'com.dendron.cuber.cube))
...
(defn in-hypercube? [length cell center]
  ; returns the given center point of
...
(defn where-is-cell "http://www.youtube.com/watch?v=JwRzi-E1l40" [cell origin N]
  (let [anchors (anchor-slots origin N)
        d-cell (map double cell)
        length (double (/ N 4))]
    (if (some #(>= %1 N) (map - d-cell origin)) :outside
      (if (not= -1 (.indexOf anchors cell))
        :anchor
        ; check quadrants in parallel, if nil, it's a border cell
        (if-let [center-q (some identity (map #(in-hypercube? length d-cell %1)
...
(defn sum-borders
  "Warning: presently intractible for dense cubes.
